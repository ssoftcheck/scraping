...
while not expired:
    ...
    datetimes = [dateparse.parse(x.find('time', {'class':'date subtle small'}).attrs['datetime']) for x in reviews]
    ...
    subratings = [x.find('div', {'class':'subRatings module'}) for x in reviews]
    ...
    recommend = [x.find('div',{'class':'flex-grid recommends'}) for x in  reviews]
    ...
    ceo_approve = [x.find('span',{'class':'showDesk'}, string='Approves of') if x is not None else None for x in recommend]
    ceo_neutral = [x.find('span',{'class':'showDesk'}, string='No opinion of') if x is not None else None for x in recommend]
    ceo_disapprove = [x.find('span',{'class':'showDesk'}, string='Disapproves of') if x is not None else None for x in recommend]
    ...
    longevity = list(map(lambda x: x.find('p',{'class':'tightBot mainText'}), reviews))
    longevity = [re.findall(r'at [\w\d]+ (.+)',x.text.replace('\xa0',u' '), re.IGNORECASE+re.DOTALL)[0] if x is not None else '' for x in longevity]

    text_main = list(map(lambda x: x.find('div', {'class':'tbl fill prosConsAdvice truncateData'}), reviews))
    text_pros = list(map(lambda x: x.find('p', {'class':'pros mainText truncateThis wrapToggleStr'}), text_main))
    text_pros = [' '.join([str(y) for y in x.contents]) if x is not None else '' for x in text_pros]
    text_cons = list(map(lambda x: x.find('p', {'class':'cons mainText truncateThis wrapToggleStr'}), text_main))
    text_cons = [' '.join([str(y) for y in x.contents]) if x is not None else '' for x in text_cons]
    text_advice = list(map(lambda x: x.find('p', {'class':'adviceMgmt mainText truncateThis wrapToggleStr'}), text_main))
    text_advice = [' '.join([str(y) for y in x.contents]) if x is not None else '' for x in text_advice]
    ...
    next_page = content.find('li', {'class':'next'}).find('a')
    if next_page is not None:
        get_url = parent_url.format(next_page.attrs['href'])
